...
export function generateCSV(transactions: Transaction[], scopes: ScopeWithIcon[]): string {
  ...
  const rows = transactions.map(tx => {
    ...
    return [date, category, amount, description].map(value => `"${value.replace(/"/g, '""')}"`).join(',');
  });
